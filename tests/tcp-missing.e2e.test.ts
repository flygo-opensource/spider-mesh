...
test('tcp rpc timeout e2e', async () => {
    const result = await runBunScript(['run', 'examples/tcp-timeout-test.ts'], 30000)
...
test('tcp rpc fallback e2e', async () => {
    const result = await runBunScript(['run', 'examples/tcp-fallback-test.ts'], 20000)
...
// Provider crash / offline detection / Phát hiện provider bị crash
// Input:  Client gọi thành công, sau đó provider bị kill, client gọi lại
// Output: Lần 1 thành công; lần 2 nhận MICROSERVICE_OFFLINE
test('tcp provider offline detection e2e', async () => {
    const result = await runBunScript(['run', 'examples/tcp-offline-test.ts'], 30000)
...
// Input:  10 RPC calls song song đến cùng một provider
// Output: Tất cả 10 kết quả đúng, không mất response nào
test('tcp concurrent rpc e2e', async () => {
    const result = await runBunScript(['run', 'examples/tcp-concurrent-test.ts'], 30000)
...
test('tcp failover: 3 nodes then 1 offline e2e', async () => {
    const result = await runBunScript(['run', 'examples/tcp-failover-test.ts'], 45000)
