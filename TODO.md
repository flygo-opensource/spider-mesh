...
- [x] Chuyển discovery transporter contract sang `@spider-mesh/discovery`: `Observable<DiscoveryMessage<T>>` + `broadcast(message)`.
- [x] Chuẩn hóa `DiscoveryMessage<T>` trong package discovery gồm `node_id`, `namespace`, `tags`, `version`, `created_at`, `seq`, `data`.
- [x] Chuẩn hóa discovery constructor options: `namespace` và `tags` bắt buộc, `node_id?` tùy chọn.
- [x] Dùng `bindMeshDiscovery()` để map `SpiderMeshNode` vào `DiscoveryMessage<SpiderMeshNode>` qua `data`.
...
- [x] Test bridge trong `@spider-mesh/discovery` từ `SpiderMeshNode` sang `DiscoveryMessage<SpiderMeshNode>`.
