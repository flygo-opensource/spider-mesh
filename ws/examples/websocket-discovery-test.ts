...
async function main() {
    ...
    const observer = start('observer', 'examples/websocket-discovery-observer.ts', env)
    await waitForOutput(observer, /WebSocket discovery observer ready/, 5000, 'observer')

    // Tiến trình 1: chỉ có ServiceA
    const p1 = start('provider-1', 'examples/websocket-discovery-provider.ts', {
        ...env,
        SERVICES: 'a',
    })
    await waitForOutput(p1, /WebSocket discovery provider ready/, 5000, 'provider-1')

    // Tiến trình 2: có cả ServiceA và ServiceB
    const p2 = start('provider-2', 'examples/websocket-discovery-provider.ts', {
        ...env,
        SERVICES: 'a,b',
    })
    await waitForOutput(p2, /WebSocket discovery provider ready/, 5000, 'provider-2')
