...
async function main() {
    ...
    const server = start('server', 'examples/websocket-e2e-reverse-server.ts', { WS_URL: wsUrl })
    await waitForOutput(server, /reverse e2e server connected/, 5000, 'server')

    const client = start('client', 'examples/websocket-e2e-reverse-client.ts', { WS_URL: wsUrl })
    await waitForOutput(client, /reverse e2e client connected/, 5000, 'client')

    const output = await waitForOutput(server, /hello from server from client/, 12000, 'server')
