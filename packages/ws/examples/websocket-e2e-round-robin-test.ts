...
async function main() {
    ...
    const client = start('client', 'examples/websocket-e2e-round-robin-client.ts', { WS_URL: wsUrl })
    await waitForOutput(client, /round-robin client connected/, 5000, 'client')

    const providerA = start('provider-a', 'examples/websocket-e2e-provider.ts', {
        WS_URL: wsUrl,
        PROVIDER_ID: 'provider-a',
    })
    await waitForOutput(providerA, /provider ready/, 5000, 'provider-a')

    const providerB = start('provider-b', 'examples/websocket-e2e-provider.ts', {
        WS_URL: wsUrl,
        PROVIDER_ID: 'provider-b',
    })
    await waitForOutput(providerB, /provider ready/, 5000, 'provider-b')

    const output = await waitForOutput(client, /"results":\[/, 12000, 'client')
